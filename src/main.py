...
import random
sys.path.append("..")
from cdrgn import utils
from cdrgn.utils import build_sub_graph
from src.rrgcn import RecurrentRGCN
from src.hyperparameter_range import hp_range
import torch.nn.modules.rnn
from collections import defaultdict
from cdrgn.knowledge_graph import _read_triplets_as_list
# os.environ['KMP_DUPLICATE_LIB_OK']='True'
from helper import get_logger, get_time_interval, get_valid_test_time_num


def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, mode, logger, dataset):
    """
    ...
    add_num = get_valid_test_time_num(mode, dataset)
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, test_triples_input, use_cuda, time_idx + add_num)
    ...
    mrr_raw = utils.stat_ranks(ranks_raw, "raw_ent", logger)
    mrr_filter = utils.stat_ranks(ranks_filter, "filter_ent", logger)
    mrr_raw_r = utils.stat_ranks(ranks_raw_r, "raw_rel", logger)
    mrr_filter_r = utils.stat_ranks(ranks_filter_r, "filter_rel", logger)
    return mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r


def run_experiment(args, logger, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    time_interval = get_time_interval(data.train)
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            # random.shuffle(idx)
            time_list = ((torch.tensor(idx) / torch.tensor(max(idx), dtype=torch.float)) * args.scale)

            for train_sample_num in tqdm(idx):
                if train_sample_num == 0: continue
                output = train_list[train_sample_num:train_sample_num+1]
                # neural SDE
                loadadjs = open(f'../data/{args.dataset}/adj_tKG.pkl', 'rb')
                ...
                if train_sample_num - args.train_history_len < 0:
                    ...
                    times.extend(time_list[0: train_sample_num])
                    for i_idx in range(0, train_sample_num):
                        edge_index_list.append(adjs[i_idx]['edge_index'])
                        edge_type_list.append(adjs[i_idx]['edge_type'])
                else:
                    input_list = train_list[train_sample_num - args.train_history_len: train_sample_num]
                    times.extend(time_list[train_sample_num - args.train_history_len: train_sample_num])
                    for i_idx in range(train_sample_num - args.train_history_len, train_sample_num):
                        edge_index_list.append(adjs[i_idx]['edge_index'])
                        edge_type_list.append(adjs[i_idx]['edge_type'])
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, output[0], static_graph, use_cuda, times, edge_index_list, edge_type_list, train_sample_num)
            ...
            logger.info("Epoch {:04d} | Ave Loss: {:.4f} | entity-relation-static:{:.4f}-{:.4f}-{:.4f} Best MRR {:.4f} | Model {} "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--use-sde', action='store_true', help='use sde')
    ...
    parser.add_argument('--brownian_size', type=int, default=2, help='the length of Brownian Motion')
    parser.add_argument('--sde_dropout_1', type=float, default=0.3, help='dropout')
    parser.add_argument('--sde_dropout_2', type=float, default=0.3, help='dropout')

    # cross-dimension mechanism
    parser.add_argument('--use-cd', action='store_true', help='use cross dimension')
    ...
    parser.add_argument('--sen_dim', type=int, default=10, help='the length of dim piece')

    # time decoder
    parser.add_argument('--use-time-decoder', action='store_true', help='use time decoder')
    parser.add_argument('--time_dropout', type=float, default=0.3, help='dropout')

    # logging information
    parser.add_argument("--log-config", type=str, default='../config/', help="logging config dir")
    parser.add_argument("--log-path", type=str, default='../log/', help='log save path')
    parser.add_argument("--name", type=str, default='00_CDRGN-SDE', help='log name')

    args = parser.parse_args()
    args.name = (args.name + '_' + time.strftime('%Y_%m_%d') + '_' + time.strftime('%H:%M:%S')).replace(':', '_')

    logger = get_logger(args.name, args.log_path, args.log_config)
