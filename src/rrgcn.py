...
# from cdrgn.layers import RGCNBlockLayer as RGCNLayer
from cdrgn.layers import UnionRGCNLayer, RGCNBlockLayer
from src.model import BaseRGCN
from src.decoder import ConvTransE, ConvTransR, ConvTransRTime, ConvTransETime
from cdrgn.nsde import NSDE
from cdrgn.cross_layer import DSW_embedding
from cdrgn.time_layer import timeRNN
...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, h_dim, opn, sequence_len, num_bases=-1, num_basis=-1,
                 ...
                 gpu=0, analysis=False, params=None, time_interval=None):
        ...
        # time representation
        self.time_net = timeRNN(self.p.dataset,  self.time_interval, self.h_dim, self.p.time_dropout)
        self.sin = torch.sin
        ...
        if self.p.use_time_decoder:
            if decoder_name == "convtranse":
                self.decoder_ob = ConvTransETime(num_ents, h_dim, input_dropout, hidden_dropout, feat_dropout)
    ...
    def forward(self, g_list, static_graph, use_cuda, times=None, edge_index_list=None, edge_type_list=None):
        gate_list = []
        degree_list = []

        if self.p.use_sde and times is not None:
            # neural SDE -> training
            for time_i in range(len(times)):
                sde = NSDE(self.dynamic_emb, self.emb_rel, self.use_cuda, self.gpu, self.p).cuda()
                sde.sdefunc.set_graph(edge_index=edge_index_list[time_i], edge_type=edge_type_list[time_i])
                sde.times = torch.tensor(times)
                sde.set_cuda()
                ent_emb, rel_emb = sde()
                self.dynamic_emb.data = ent_emb
                self.emb_rel.data = rel_emb

        if self.p.use_cd and times is not None:
            dpw = DSW_embedding(self.p.sen_len, self.h_dim, self.p.cross_dropout, self.p.sen_dim).cuda()
            dpw_emb = dpw(self.dynamic_emb)
            self.dynamic_emb.data = dpw_emb
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda, time_point):
        with torch.no_grad():
            ...
            if self.p.use_time_decoder:
                time_list = torch.tensor([time_point] * len(all_triples)).to('cuda')
                time_embs = self.get_init_time(time_list)
                score = self.decoder_ob.forward(embedding, r_emb, all_triples, time_embs, mode="test")
                score_rel = self.rdecoder.forward(embedding, r_emb, all_triples, time_embs, mode="test")
            else:
    ...
    def get_loss(self, glist, triples, static_graph, use_cuda, times, edge_index_list, edge_type_list, time_point):
        """
        :param glist:
        :param triples:
        :param static_graph:
        ...
        evolve_embs, static_emb, r_emb, _, _ = self.forward(glist, static_graph, use_cuda, times, edge_index_list, edge_type_list)
        ...
        if self.p.use_time_decoder:
            ...
            if self.entity_prediction:
                scores_ob = self.decoder_ob.forward(pre_emb, r_emb, all_triples, time_embs).view(-1, self.num_ents)
                loss_ent += self.loss_e(scores_ob, all_triples[:, 2])

            if self.relation_prediction:
                score_rel = self.rdecoder.forward(pre_emb, r_emb, all_triples, time_embs, mode="train").view(-1, 2 * self.num_rels)
    ...
    def get_init_time(self, time_list):
        T_idx = self.time_net(time_list)
        # T_idx = time_list // self.time_interval
